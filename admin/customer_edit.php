  ...
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item"><a href="#">Employers</a></li>
       ...
       <form action="" style="border:1px solid gray;width:50%; margin-left:10%; padding:10px;">
			 ...
			 $id=$_GET['id'];
			  $query=mysqli_query($conn,"select * from employer_reg where emp_id=$id");
			   while($row=mysqli_fetch_array($query)){
			  ?>
			  <h1><?php echo strtoupper($row['emp_fname']); ?> <?php echo strtoupper($row['emp_lname']); ?></h1>
			  <hr>
          
			   <div class="form-group">
			   <label for="">Employer Name:</label>
                <td><?php echo $row['emp_fname'];?>&nbsp<?php echo $row['emp_lname'];?> </td></div>
				...
				<td><?php echo $row['emp_mobile'];?></td></div>
				 ...
				 <div class="form-group">
			<label for="">Employer Username:</label>
                <td><?php echo $row['emp_username'];?></td></div>
				
				<div class="form-group">
				<label for="">Employer Role:</label>
                <td><?php echo $row['emp_role'];?></td></div>
